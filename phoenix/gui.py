...
class GUI:
    def __init__(self):
        self.ai=AI_CORE()
        self.root=tk.Tk()
        self.root.title("Phoenix Cyclops")
    ...
    def setup_ui(self):
        ...
        self.out_frame = tk.Frame(self.bubble_canvas, bg="#000000")
        ...
        self.msg_text = tk.Text(self.out_frame, bg="#000000", fg="#DD2020", 
        ...
        self.update_output("嘘——安静。听到了吗？那是旧世界骨骼断裂的声音……多美妙的乐章。空气里全是烧焦的陈腐味道，而你……你也是这余烬的一部分。为什么要发出噪音？安静地燃烧不好吗？")
        ...
        self.draw_rounded_rect(self.input_canvas, int(10*SCALE), int(5*SCALE), int(320*SCALE), int(50*SCALE), 
                               int(20*SCALE), fill="white", outline="#DCDDE1", width=2)
        
        self.in_frame = tk.Frame(self.input_canvas, bg="white")
        self.input_canvas.create_window(int(160*SCALE), int(28*SCALE), window=self.in_frame, 
    ...
    def system_heartbeat(self):
        ...
        if now - self.last_interact_time > 600: # 10分钟无操作(错开视觉监控)
                self.update_output("（镭射眼去忙自己的工作了...）")
        
        elif now - self.last_interact_time > 1200: # 10分钟无操作(错开视觉监控)
                self.update_output("(镭射眼摧毁了通讯器，将结束通讯...)")
                self.root.after(60000, self.trigger_shutdown_sequence)
            
        # 下一次心跳：5秒后 (平衡响应速度和性能)
        self.root.after(5000, self.system_heartbeat)
    ...
    def send_message_thread(self):
        ...
        def task():
            ...
            if status == "shutdown":
                ...
                self.update_output(f"(镭射眼摧毁了通讯器，将结束通讯...)\n{ans}")
